import { expectMatrixEq, expectEq, expectNotEq, expectFalse, expectTrue, Tester, expectSame } from "./tester";
...
import { counterExample, exhaustive, specialized, useful, uselessClause } from './useful';
...
// Scratchpad
document.body.appendChild(tag('', 'P:', 'h3'));
document.body.appendChild(matrixToHtml(P));
...
export default function runTests(container) {
  ...
  uselessSuite.case('P has no useless clause', () => expectSame(uselessClause(P), null));
  uselessSuite.case('Q has useless clause at end', () => expectSame(uselessClause(Q), 5));

  // Counter-examples
  const counterExampleSuite = t.addSuite('Counter-Example');
  counterExampleSuite.case('P has example', () => expectTrue(counterExample(P) != null));
  counterExampleSuite.case('Q has no example', () => expectFalse(counterExample(Q) != null));
  counterExampleSuite.case('Q[:-1] has no example', () => expectFalse(counterExample(Q.sliceRows(0, -1)) != null));
  counterExampleSuite.case('Q[:-2] has example', () => expectTrue(counterExample(Q.sliceRows(0, -2)) != null));
